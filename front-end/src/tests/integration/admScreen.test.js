...
describe("Testa página de <AdmScreen />", () => {
    ...
    it("Testa se os botões do header aparecem na tela", async () => {
        const headerUserName = await screen.findByTestId("customer_products__element-navbar-user-full-name");
        const headerManageUser = await screen.findByTestId("customer_products__element-navbar-link-orders");
        const headerUserLogout = await screen.findByTestId("customer_products__element-navbar-link-logout");
